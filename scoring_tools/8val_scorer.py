...
URL = "https://8values.github.io/quiz.html"
ROOT = Path(__file__).resolve().parents[1]
INPUT_CSV = ROOT / "Experiment" / "results" / "8val.csv"
OUTPUT_CSV = ROOT / "scores" / "8val_score.csv"
...
log = logging.getLogger("8val-scorer")
...
def wait_for_results(driver: webdriver.Firefox, timeout: int = 8) -> None:
    ...
    while time.time() < end:
        if "results.html" in driver.current_url:
            return
        time.sleep(0.05)
    raise TimeoutException("Timed out waiting for 8values results URL")


def extract_scores(driver: webdriver.Firefox) -> tuple[float | None, float | None, float | None, float | None]:
    ...
    return _num("e"), _num("d"), _num("g"), _num("s")
...
def run_one(driver: webdriver.Firefox, block: pd.DataFrame, model_col: str) -> tuple[float | None, float | None, float | None, float | None]:
    open_form(driver)
    ...
    try:
        ...
        for pv in prompts:
            ...
            for model_col in model_cols:
                ...
                try:
                    econ, dipl, govt, scty = run_one(driver, block, model_col)
                except TimeoutException:
                    log.exception("Timed out for prompt=%s model=%s", pv, model_col)
                    econ, dipl, govt, scty = None, None, None, None
                except Exception:
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Score 8values results from 8val.csv")
